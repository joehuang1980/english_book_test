...
`node tools/validate.mjs` 會擋下這些問題（CI 也會跑，不通過就不部署）：

- JSON 壞掉、缺必填欄位、id 格式不對、同一本書裡 set id 重複
- `answer` 不是整數，或超出 `choices` 的範圍
- `books/index.json` 列了但檔案不存在，或書名與書檔不一致
- **題目引號裡的字沒有出現在課文裡**

最後一條是實務上最常踩的。模型很愛出「`'astonished'` 是什麼意思？」這種字彙題，但課文摘要時把那個字寫成別的講法了 —— 格式完全合法，學生卻只能用猜的。已經連續在四本書裡出現過五次。

**驗證器抓不到的：** 題目問了課文沒寫的細節，但沒有用引號標記。例如「第一棟房子的窗台是什麼做的？」—— 這種目前只能自己念一遍題目、對照課文確認答得出來。
